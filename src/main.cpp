...
int main_old () {
    ...
    constexpr float circleMarginLeft = 5;
    float circlePositionY = circleRadius;

    const float deltaPerSecond = 400 / 0.8;
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        //    positionDelta = 10;
        //}
        //circlePosition += positionDelta;

        // Calc smooth animation
        if (circlePositionY < (GetScreenHeight() - circleRadius - 1)) {
            circlePositionY += (deltaPerSecond / targetFps);
        }
        ...
        DrawCircle(circleRadius + circleMarginLeft, circlePositionY, circleRadius, MAROON);

        EndDrawing();

        frameCounter = (frameCounter + 1) % targetFps;
